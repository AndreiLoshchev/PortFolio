...
@application.route('/multinput/')
def multiget():
    return render_template ("multiinput.html")
@application.route('/multinput/',methods=['POST'])
def multi_input():
        shop_Id = request.form['shop_ids']
        shopID=[int(x) for x in shop_Id.split(',')]
        data = pd.read_csv('weekly sales and labour cost for all shops 2013 to 20177.csv',index_col='start_date', parse_dates=True)
        # shopID = input("Enter your shop id")
        tab_f = pd.DataFrame(columns=['shop_id', 'avg_wk_sales', 'max_sale', 'min_sale', 'avg_itemsold', 'item_price'])
        shopID1 = list(shopID)
        for j in shopID1:
            plt.style.use('ggplot')
            ...
            if len(df2.week_no) > 51:
                ...
                tab1 = pd.DataFrame({"shop_id": df2.shop_id.mean(), 'avg_wk_sales': round(df2.sales_amount.mean(), 2),
                                     'max_sale': round(max(df2.sales_amount), 2),
                                     'min_sale': round(min(df2.sales_amount), 2),
                                     'avg_itemsold': round(df2.item_sold.mean(), 2),
                                     'item_price': round((df2.sales_amount / df2.item_sold).mean(), 2)}, index=[0])
                tab_f = tab_f.append(tab1)
                ...
                data1f = pd.read_csv('weekly sales and labour cost for all shops 2013 to 20177.csv')

                dff1f = data1f[data1f.sales_status != 0]
                nulldetect = dff1f.week_no.isnull()
                dff1f.week_no.loc[nulldetect == True] = 54
                dff1f['week_no'] = dff1f.week_no - 2
                X_Cluster = dff1f[['shop_id', 'sales_amount']]
                from sklearn.cluster import KMeans
                kmeans_model = KMeans(n_clusters=3, random_state=8).fit(X_Cluster)
                y_hat = kmeans_model.labels_  # clusters
                cen = kmeans_model.cluster_centers_
                ...
                if (group_low_sales2.shop_id.values == q).any() == True:
                    ...
                    trt = group_low_sales[group_low_sales.sales_amount > cx - 3000]
                    trt2 = trt[trt.sales_amount < cx + 3000]
                    valid_cls = dff1f[['sales_amount', 'item_sold', 'transactions', 'week_no']].loc[trt2.index.values]
                ...
                figg.set_size_inches(13, 7)
                # print(valid_cls)
                tab2 = pd.DataFrame({"shop_id": df2.shop_id.mean(), 'avg_wk_sales': round(df2.sales_amount.mean(), 2),
                                     'max_sale': round(max(df2.sales_amount), 2),
                                     'min_sale': round(min(df2.sales_amount), 2),
                                     'avg_itemsold': round(df2.item_sold.mean(), 2),
                                     'item_price': round((df2.sales_amount / df2.item_sold).mean(), 2)}, index=[0])
                tab_f = tab_f.append(tab2)
        ...
        #figg=mpld3
        tab_f['shop_id'] = tab_f.shop_id.astype(int)
        tab_f = tab_f[['shop_id', 'avg_wk_sales', 'max_sale', 'min_sale', 'avg_itemsold', 'item_price']]
        tab_f.to_json(path_or_buf='tab_f.json', orient='records')
...
#@app.route('/multi/'.)
#def multi_input1():
        return render_template('multinput1.html',graphh=graphh,value2=shopID1)

#fan hui json fang fa
@application.route('/tablejson', methods=['GET', 'POST'])
def tablejson():
    import codecs
...
@application.route('/multitabjson', methods=['GET', 'POST'])
def multitabjson():
    import codecs
    fd1 = codecs.open('tab_f.json')
    df1=fd1.read()
    fd1.close()
    print(df1)
    return df1
...
@application.route('/df_tabb', methods=['GET', 'POST'])
def memval2():
    print(memval2)
    return memval2
...
@application.route('/multitable/')
def multitable():
    return render_template('multitab.html')
